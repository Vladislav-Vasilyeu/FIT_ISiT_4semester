...
BEGIN
  MakeBooking(p_UserId => 61, p_TourId => 21);
END;
/
...
BEGIN
  CancelBooking(p_BookingId => (SELECT MAX(BookingId) FROM Bookings WHERE UserId = 61 AND TourId = 21));
END;
/
...
SELECT * FROM View_BookingDetails WHERE ClientName LIKE '%10%';
